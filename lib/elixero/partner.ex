defmodule EliXero.Partner do
  ...
  def get_request_token(config) do
    ...
    header = EliXero.Utils.Oauth.create_auth_header(config, "GET", request_token_url, [oauth_callback: callback_url])
  ...
  def approve_access_token(config, request_token, verifier) do
    ...
    header = EliXero.Utils.Oauth.create_auth_header(config, "GET", access_token_url, [oauth_token: request_token["oauth_token"], oauth_verifier: verifier])
  ...
  def renew_access_token(config, access_token) do
    access_token_url = EliXero.Utils.Urls.access_token
    header = EliXero.Utils.Oauth.create_auth_header(config, "GET", access_token_url, [ oauth_token: access_token["oauth_token"], oauth_session_handle: access_token["oauth_session_handle"] ])
    response = EliXero.Utils.Http.get(config, access_token_url, header)
  ...
  def upload_multipart(config, access_token, resource, api_type, path_to_file, name) do
    ...
    header = EliXero.Utils.Oauth.create_auth_header(config, "POST", url, [oauth_token: access_token["oauth_token"]], name)

    EliXero.Utils.Http.post_multipart(config, url, header, path_to_file, [{"Name", name}])
  end
